...
"
  (let ((hash (make-hash-table))
    ...
    (labels ((set-tag (tag lines)
               (setf (gethash (intern (string-upcase (svref (cdr tag) 0))) hash)
                     lines))
             (push-lines ()
               (unless (or (null lines) (null tag))
                 (set-tag tag
                          (reduce (lambda (a b) (concatenate 'string b (string #\newline) a))
                                  (trim-empty-lines lines)))))
             (push-empty-line () (push "" lines))
             (block-tag-p (tag)
               (and (not (null tag)) (eq :block (car tag)))))
      (loop
            ...
            (etypecase parsed
              ...
              (cons (let ((tagtype (car parsed)))
                      ...
                      (setf lines nil
                            tag nil)
                      (case tagtype
                        (:oneline (set-tag parsed
                                           (svref (cdr parsed) 1)))
                        (:block (setf tag parsed)))))
              (string (when (block-tag-p tag)
                        (push parsed lines)))))
      hash)))
